...
class SauceNaoEngine(GenericRISEngine):
    ...
    def best_match(self, url: str | URL) -> tuple[dict[str, str | int | URL], list[InlineKeyboardButton]]:
        ...
        results = filter(lambda d: float(d["header"]["similarity"]) >= 75, response.json().get("results", []))
        ...
        if ext_urls := data["data"].get("ext_urls"):
            ...
            buttons = [InlineKeyboardButton(text=url_icon(u, True, True), url=str(u)) for u in urls]  # type: ignore
            for u in urls:
