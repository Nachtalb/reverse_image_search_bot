...
class TestRedisStorageDateTypesMixin:
    ...
    async def test_set_dict(self) -> None:
        ...
        self.redis.redis_client.set.assert_awaited_with("ris:sj:test_key_dict", json_data)

    async def test_get_dict(self) -> None:
        test_dict = {"key": "value"}
        self.redis.redis_client.get.return_value = json.dumps(test_dict)
        result = await self.redis.get("ris:sj:test_key_dict")
    ...
    async def test_set_list(self) -> None:
        ...
        self.redis.redis_client.set.assert_awaited_with("ris:sj:test_key_list", json_data)

    async def test_get_list(self) -> None:
        test_list = ["one", "two", "three"]
        self.redis.redis_client.get.return_value = json.dumps(test_list)
        result = await self.redis.get("ris:sj:test_key_list")
    ...
    async def test_get_with_incorrect_data_type(self) -> None:
        ...
        with pytest.raises(ValueError):
            await self.redis.get("ris:sb:test_key_bool")
    ...
    async def test_keys_with_valid_custom_pattern(self) -> None:
        pattern = "custom:*"
        expected_keys = ["ris:ss:custom:1", "ris:si:custom:2"]
        self.redis.redis_client.keys.return_value = expected_keys
        result = await self.redis.keys(pattern)
        self.redis.redis_client.keys.assert_awaited_with("ris:[sx][sifbj]:custom:*")
        assert result == expected_keys

    async def test_keys_without_ris_prefix(self) -> None:
        pattern = "key_without_prefix"
        expected_keys = ["ris:ss:key_without_prefix"]
        self.redis.redis_client.keys.return_value = expected_keys
        result = await self.redis.keys(pattern)
        self.redis.redis_client.keys.assert_awaited_with(f"ris:[sx][sifbj]:{pattern}")
        assert result == expected_keys

    async def test_check_key_format_valid(self) -> None:
        key = "ris:ss:valid_key"
    ...
    async def test_serialize_bool(self) -> None:
        assert self.redis._serialize(True, "ris:sb:key") == "1"

    async def test_serialize_int(self) -> None:
        assert self.redis._serialize(1, "ris:si:key") == "1"

    async def test_serialize_float(self) -> None:
        assert self.redis._serialize(1.0, "ris:sf:key") == "1.0"
    ...
    async def test_serialize_list(self) -> None:
        ...
        assert self.redis._serialize(list_value, "ris:sj:key") == json.dumps(list_value)

    async def test_serialize_dict(self) -> None:
        dict_value = {"key": "value"}
        assert self.redis._serialize(dict_value, "ris:sj:key") == json.dumps(dict_value)
    ...
    async def test_deserialize_int(self) -> None:
        assert self.redis._deserialize("1", "ris:si:key") == 1

    async def test_deserialize_float(self) -> None:
        assert self.redis._deserialize("1.0", "ris:sf:key") == 1.0

    async def test_deserialize_bool(self) -> None:
        assert self.redis._deserialize("1", "ris:sb:key") is True

    async def test_deserialize_dict(self) -> None:
        json_value = '{"key": "value"}'
        assert self.redis._deserialize(json_value, "ris:sj:key") == {"key": "value"}
    ...
    async def test_mget_dict(self) -> None:
        ...
        assert result == {"ris:ss:key1": "value1", "ris:si:key2": 42}
